function [mih,plotHandle] = frequency_dwellFli7(varargin)
...
%
% is identical to v5, but computes the event frequenciy slightly
% differently. It find the freq starting with the end of first event. This
% is appropriate for calculating idle periods, but not for plotting
...
mih.sints = sints;
% totalFrequency = length(events)/low_times; % this counts the first event in the freq calc, which is fine for the plot, but not in reporting idel periods, as we already account for the first event in the first passage plot
totalFrequency = length(low_events)/low_times; % This looks at frequency following the end of the first event. Which is appropriate for reporting idle times. 
...
%     plotHandle = plot(tfit,SPfit*totalFrequency,'Color',[0.4 0.4 0.4]);shg
    plotHandle = plot(tfit,SPfit*totalFrequency,'--','Color',colr,'LineWidth',1);shg
    mih.k1=params(2);
    mih.k2=params(3);
    mih.a = a;
end
