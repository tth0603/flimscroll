function [mih,plotHandle] = frequency_dwellFli2(varargin)
...
%
% this version uses *basically* the end of the last event the upper bound
% for calculating event frequency. This makes it hard to compare between
% replicates because I turn the microscope off at different times. See v3
% for a different way. 
%
...
if ~isempty(varargin{numOfReps + 4})
    ...
    corr=expfalltwo_mxl_correction(inarg,events,tm,tx);      %correction factors for the fact that want to characterize the entire phenomena, but only observe it for a part of the time
        %'corr' is a struc: 
