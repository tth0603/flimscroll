function [mih,plotHandle] = frequency_dwellFli4(varargin)
% [mih,plotHandle] = frequency_dwellFli4(cia1,cia2,timestep,colr,fignum,fitParam[optional,singleExp])
%
...
%
%
% This version fits to a single exp.
%
...
%
%  fitParam == scaler for single exp fit. pass via [].
%
%version 3.2 (Nov 2015)
%updated Harden 2018 for flimscroll data
                                                                 
%Defining vars
repNum = length(varargin) - 4;
timestep = varargin{repNum + 1};
colr = varargin{repNum + 2};
fignum = varargin{repNum + 3};
sints=[]; 
low_events = [];
events = [];
expLength = zeros(2,1);
...
for i = 1:repNum
    ...
    firstTime = min(firstTimeV); % this is only relevant if we are including time before first event
...
%         intervals = diff(mat2(:,7)) - mat2(1:end-1,6) - 60; % start time of next event minus start time of this event plus this even't duration. The -60 is to get rid of the lag at the start of the survival curve
    ...
    %     strtInt = mat2(1,7) - firstTime; %time of this nukes first event minus the time we first detect a spot in this experiment
        strtInt = []; % ATM we dont ijnclude the times before the first event
...
mih.sints = sints;
totalFrequency = length(events)/low_times;
mih.totalFrequency = totalFrequency;
mih.eventNumber = length(events);
mih.lowTimes = low_times;
mih.expLength = expLength;

if ~isempty(varargin{repNum + 4})
    %from survivalPLotFromVector.m with som revisions
    inarg = varargin{6};
    tm = 1;tx = 3000;
    params=fminsearch('expfallone_mxl',inarg,[],events,tm,tx);
    k1=abs(params); %not sure why we have to use abs
    corr=expfallone_mxl_correction(k1,events,tm,tx);      %correction factors for the fact that want to characterize the entire phenomena, but only observe it for a part of the time
        %'corr' is a struc: 
    ...
    tfit=[0:max(events)];
    SPfit=corr.CorrectionFactor*exp(-tfit*k1);  %This is the survival 
...
%     SPfit = a*k1*exp(-tfit*k1)+(1-a)*k2*exp(-tfit*k2);
    hold on;
    plotHandle = plot(tfit,SPfit*totalFrequency,'--','Color',colr);shg
    mih.k1=params;
end
