    function [mib,plotHandle] = survivalPlotFromVector(data,col,fit,tm,tx,figNum,varargin)
% [mib,plotHandle] = freqSurvivalPlotFromVector(data,col,fit,tm,tx,figNum,varargin)
%
...
%   varargin{1} = the initial guesses for your fit params. only applies for
...
%   guess].
%   varargin{2} = a color vector. If you include this, the fit line will be
%   this color
%   
%   OUTPUT
...
if fit == 1
    ...
    if ~isempty(figNum)
        ...
        if isempty(varargin{2})
            ...
            plot(tfit,log(SPfit),'--','Color',varargin{2},'LineWidth',1);shg
        end
    else 
...
%     plotHandle = stairs(sints(:,1),log((sints(:,2))/n),'LineWidth',1);
    plotHandle = stairs(sints(:,1),log((sints(:,2))/max(sints(:,2))),'LineWidth',1);
