function pc=exp2_global_active_fraction_nonspecific_background_mxl(inarg,datacells,rc)
%
...
%     rate1=inarg(1+indx);                     % landing rate for this data set
                                    % Cycle through all the data sets
            % List of probabilities for the observed landings
%     probints = ( (Af*Nt - Nz)/Nt )*(rate1+rc)*exp(-(rate1+rc)*intervals) + (1-Af)*rc*exp(-rc*intervals);   
%     probints = ( (Af*Nt - Nz)/Nt )*(rate1+rc)*(exp(-(rate1+rc)*intervals) + exp(-(rate2 + rc)*intervals)) + (1-Af)*rc*exp(-rc*intervals);
%     probints = ( (Af*Nt - Nz)/Nt )*rate1*rate2/(rate1 - rate2)*(exp(-(rate2+rc)*intervals) - exp(-(rate1 + rc)*intervals)) + (1-Af)*rc*exp(-rc*intervals);
    probints = rate1*rate2/(rate1 - rate2)*(exp(-(rate2+rc)*intervals) - exp(-(rate1 + rc)*intervals));% + (1-Af)*rc*exp(-rc*intervals);
        %      (#active past t=0)/(total AOI#) *rate*exp( )

        % Probability for observing one instance of AOI w/o a landing 
%     probNs = ( ((Nt - Af*Nt)/Nt)*exp(-tx*rc) + ( (Af*Nt - Nz)/Nt )*exp(-tx*(rate+rc)) );   %TH add this back lateer
