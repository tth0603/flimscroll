function [mib,plotHandle] = freqSurvivalPlotFromVector(data,col,fit,tm,tx,figNum,varargin)
% [mib,plotHandle] = freqSurvivalPlotFromVector(data,col,fit,tm,tx,figNum,fitParams,low_times)
%
% THIS IS A MESS, harden 201126
% 
...
%   fitParams = the initial guesses for your fit params. only applies for
...
%   guess].
%   low_times == from LJF's frequency_dwellFli2. the sum of all the off
%   times across all AOIs over an experiment
%   
%   OUTPUT
...
if fit == 1
    ...
    if ~isempty(figNum)
        ...
        plotHandle = plot(sints(:,1),log((sints(:,2)+corr.Offset)/corr.Nzero),'LineWidth',1);
        hold on;
        plot(tfit,log(SPfit),'Color',[0.4 0.4 0.4]);shg
    else 
        plotHandle = 1; %do this only to avoid an error
    end
...
%         plotHandle = plot(sints(:,1),log((sints(:,2)+corr.Offset)/corr.Nzero),'LineWidth',1);   
        hold on;
%         plotHandle = plot(tfit,SPfit/n,'Color',[0.4 0.4 0.4]);shg
        plotHandle = plot(tfit,SPfit/(low_times/n),'Color',[0.4 0.4 0.4]);shg
        %plot(tfit,SPfit,'g-');
    else 
        plotHandle = 1; %do this only to avoid an error
    end
