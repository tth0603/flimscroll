    function [mib,plotHandle] = survivalPlotFromVectorTaus(data,col,fit,tm,tx,figNum,varargin)
...
%   exponential. Uses taus, not rates. A BIT OF A MESS on 201201
...
elseif fit==2       %an example of a proper way to fit survival curves with a double exponential max likelihood
    ...
    params=fminsearch('expfalltwo_mxlTaus',inarg,[],intervals,tm,tx); 
...
%     a=params(1);
    tau1=params(2);
    tau2=params(3);  %defines rate constants for fit

    tfit=[0:max(intervals)];

    SPfit=a*exp(-tfit/tau1)+(1-a)*exp(-tfit/tau2);  %This is the survival 
                                                %curve model
