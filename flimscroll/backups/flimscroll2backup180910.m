...
%           this is also where spots from feature picker are stored    
...
% Last Modified by GUIDE v2.5 10-Sep-2018 15:38:07
...
%If we want to show nuclei. If we've tracked those spots, have them follow along:
%get stuff:
if get(handles.displayNuclei,'Value') == 1
    FitData = get(handles.pick,'UserData');
    aoifits = get(handles.trackNuclei,'UserData');
    %try first for tracking, then for the location
    ...
    elseif ~isempty(FitData)
        aoirose = size(FitData,1);
        for indx=1:aoirose
            viscircles(FitData(indx,3:4),pixnum/2,'Color','y','LineWidth',0.5,'EnhanceVisibility',false);
            if shoNum == 1
...
if get(handles.plotSpots,'Value') == 1
    %for paired spots
    spotPairs = get(handles.makeMatches,'UserData');  %this is where the matched spots struc lives. current form: [frame nucleus binarySpot distance xPos yPos]
    %for regular spots
    spots = get(handles.findAllSpots,'UserData');
    
    if ~isempty(spotPairs)
        %this is where the matched spots struc lives. current form: [frame nucleus binarySpot distance xPos yPos]
        spotsLogi = spotPairs(:,1) == imagenum;
        ...
        for i = 1:frameSpotsLen
            ...
            if shoNum == 1
                text(currentSpots(i,5)+3,currentSpots(i,6)+3,num2str(currentSpots(i,2)),'FontSize',8,'Color','r'); %x & Y shifted by 3 pix, write out the aoi number
            end
        end
        hold off
    elseif ~isempty(spots)  %place plotting regular spots here
        ...
        set(handles.instrucs,'String','a work in progress')
    end
...
set(handles.maxSlider,'Max',255);
set(handles.maxSlider,'Value',255);
set(handles.maxSlider,'SliderStep',[1/(255) 10/(255)]);

set(handles.minSlider,'Min',0);
set(handles.minSlider,'Max',255);
set(handles.minSlider,'Value',0);
set(handles.minSlider,'SliderStep',[1/(255) 10/(255)]);

set(handles.maxScale,'String',255);
...
set(handles.instrucs,'String','tracking. this may take a moment.')
pause(0.1)
...
mapstruc2d=build_2d_mapstruc_aois_frmsFli(folder,pixnum,frms,ave,FitData);        % Build a 2D mapstruc to direct data processing  
...
set(handles.makeMatches,'UserData',pairedSpots);
