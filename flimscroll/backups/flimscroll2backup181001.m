...
%       spot-nuclei pairs: handles.makeMatches,'UserData' (this is presently a struc called analData with a spotsMat & spotsMatDescription. made in script 'matchMaker')
...
% Last Modified by GUIDE v2.5 01-Oct-2018 16:28:51
...
if isempty(timeBase)
    ...
    pause(0.1)
    analData = matchMaker(nukes,spots);
else 
    set(handles.instrucs,'String','Pairing spots with nuclei')
    set(handles.makeMatches,'String','...')
    pause(0.1)  
    analData = matchMaker(nukes,spots,timeBase);
end
...
if ischar(fn)  %load, if uninterrupted  
    ...
    set(handles.instrucs,'String','AP position file loaded')
    cd(currDir);
else
...
%update description:
analData.spotsMatDescription = '[1.time 2.frameNum 3.nucleus 4.distanceFromNucleus 5.xPos 6.yPos 7.amplitude 8.sigma 9.offset 10.intensity 11.relativeFrameNumber 12.APpos]';
%save to GUI data
