function varargout = flimscroll(varargin)
% FLIMSCROLL Application M-file Gofor imscrollmou.fig
%    FIG = FLIMSCROLL launch flimscroll GUI.
%    FLIMSCROLL('callback_name', ...) invoke the named callback.

% Last Modified by GUIDE v2.5 23-Aug-2018 12:43:07
...
% along with this softvware. If not, see <http://www.gnu.org/licenses/>.
...
%     load filelocations.dat -mat; 
                                            ...
                                            %FileLocations.flimscroll
                                                %FileLocations = 
                                                %     data: 'p:\matlab12\larry\data'
                                                %     avis: 'p:\matlab12\larry\avis'
                                                %     mapping: 'p:\matlab12\larry\fig-files\flimscroll\mapping'
                                                %     mapping:'p:\matlab12\larry\fig-files\flimscroll'
...
%                              (pixnum-1)/2,'b')
    ...
    for indx=1:maoi
        ...
        if get(handles.StartParameters,'Value') == 5
            data = handles.data;
            XYshift = waltzing(data, val, indx, aoiinfo);
        end
        ...
        if any(get(handles.FitChoice,'Value')==[5 6]) 
            ...
            viscircles(aoiinfo(indx,3:4)+XYshift,pixnum/2,'Color','b','LineWidth',0.5,'EnhanceVisibility',false);
        else
...
elseif argnum==2                                        % load an AOI set
    filestring=get(handles.OutputFilename,'String');
     ...
     if isfield(aoifits,'data')
         handles.data = aoifits.data;
     end
...
elseif argnum==8    
   ...
   %eval(['load ' handles.FileLocations.flimscroll '\DriftInfo.dat -mat']);
   %eval(['load ' handles.FileLocations.flimscroll 'DriftInfo.dat -mat']);
   eval(['load ' handles.FileLocations.gui_files 'DriftInfo.dat -mat']);
                                          % Load file in 'flimscroll' directory containing parameters
   ...
   % eval(['save ' handles.FileLocations.flimscroll '\DriftList.dat driftlist']);
   % eval(['save ' handles.FileLocations.flimscroll '\DriftListIntermediates.dat datt']);
     % eval(['save ' handles.FileLocations.flimscroll 'DriftList.dat driftlist']);
     %eval(['save ' handles.FileLocations.flimscroll 'DriftListIntermediates.dat datt']);
...
elseif argnum==12
    ...
    for indx=1:shadownum
        ...
        if get(handles.FitChoice,'Value')==5
                    ...
                    % account for possible drift
                    % Here to draw boxes that fractionally overlaps pixels
            viscircles([shadowx_vec(indx) shadowy_vec(indx)],pixnum/2,'Color','y','LineWidth',0.5,'EnhanceVisibility',false);
        else
                    % Here to draw aoi boxes only at pixel boundaries
           viscircles([shadowx_vec(indx) shadowy_vec(indx)],pixnum/2,'Color','y','LineWidth',0.5,'EnhanceVisibility',false);
        end
...
if aoirose~=0       % If there are spots, put them into handles.FitData and draw them
    ...
    handles.FitData=[imagenum*ones(aoirose,1) ave*ones(aoirose,1) pk(:,1) pk(:,2) pixnum*ones(aoirose,1) [1:aoirose]'];
                    % Draw the aois
    for indx=1:aoirose
        viscircles(handles.FitData(indx,3:4),pixnum/2,'Color','b','LineWidth',0.5,'EnhanceVisibility',false);
    end
end
...
switch MenuValue
    ...
    case 4
        ...
        eval(['save ' handles.FileLocations.mapping 'field2.dat aoiinfo2'])
                        % Load the new mapping file into flimscroll
...
% --- Executes on button press in pickNsave.
function pickNsave_Callback(hObject, eventdata, handles)
% hObject    handle to pickNsave (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
fp = handles.TiffFolder;


PickSpotsButton_Callback(hObject, eventdata, handles);
spots = handles.FitData;
